...
# Include non-python files like compiled libraries (*.so, *.dylib)
# Make this more specific and conditional
package_data = {
    '': ['*.yml', '*.md'], # Include YAML configs and READMEs
    # Conditionally include backend compiled files
    # 'orion.backend.lattigo': ['*.so', '*.dylib', '*.h'], # If building Lattigo
    # 'orion.backend.heongpu': ['*.so', '*.dylib', '*.h'], # If building HEonGPU
}
if ORION_BACKEND_TO_BUILD == 'lattigo':
...
install_requires = [
    ...
    'matplotlib>=3.1.0',
    'numpy>=1.21.0',
    'scipy>=1.7.0,<=1.14.1', # Note: Max version specified
    'torch>=2.2.0',
    'torchvision>=0.17.0',
    'tqdm>=4.30.0'
]
# Add backend-specific dependencies if any
# if ORION_BACKEND_TO_BUILD == 'heongpu':
#     install_requires.append('pybind11>=2.6') # Example if using pybind11
...
class build_ext(_build_ext):
    def run(self):
        if ORION_BACKEND_TO_BUILD == 'lattigo':
            ...
            try:
                # Assumes build_lattigo.py has a function 'build_go_library'
            ...
            # Assuming CMake is used by adrianHEonGPU
            ...
            cmake_args = [
                ...
                # Add HEonGPU specific CMake flags here if needed (e.g., CUDA paths)
                # '-DCMAKE_CUDA_COMPILER=/path/to/nvcc'
                # '-DCMAKE_BUILD_TYPE=Release',
            ]
            build_args = ['--config', 'Release', '--parallel'] # Build in parallel

            try:
                # Configure CMake for HEonGPU library
                ...
                # Build HEonGPU library
                print("--- Building HEonGPU library ---")
                subprocess.check_call(['cmake', '--build', '.'] + build_args, cwd=cmake_build_dir)

                # Install HEonGPU library (optional, if CMakeLists supports it)
                # print("--- Installing HEonGPU library ---")
                # subprocess.check_call(['cmake', '--install', '.'], cwd=cmake_build_dir)

                print("*"*10, "Finished Building HEonGPU Library", "*"*10)

                # Now build the wrapper extension (handled by super().run() below)
        ...
        # Call the original build_ext command AFTER backend lib is built
        # It will build any Extension modules defined in ext_modules
        _build_ext.run(self)

# --- Define Extensions (Only needed for C++/Cython/Pybind11 wrappers) ---
ext_modules = []
if ORION_BACKEND_TO_BUILD == 'heongpu':
     # Example assuming a pybind11 wrapper in orion/backend/heongpu/
     # This requires a corresponding CMakeLists.txt in that directory
     # to handle the pybind11 specifics and linking against libheongpu
     ext_modules.append(
         Extension(
             'orion.backend.heongpu.heongpu_backend', # name of the .so file
              ...
              include_dirs=[ # Include dirs for pybind11 and HEonGPU headers
                  # get_pybind_include(), # Function to find pybind11 headers
                  'orion/backend/heongpu/source/include', # Example HEonGPU include path
              ],
              library_dirs=[ # Where to find the compiled libheongpu.so/.a
                   os.path.abspath('orion/backend/heongpu/lib/lib') # Adjust path
              ],
              libraries=['heongpu'], # Name of the library to link against
              language='c++',
              # Add specific compiler/linker flags if needed
              # extra_compile_args=['-std=c++17'],
              # extra_link_args=[],
         )
     )
# Note: A more robust way for C++ extensions often involves using CMake
# within the build_ext command itself, rather than the basic Extension class.
# The CMakeExtension example from the previous response is often better.
...
setup_kwargs = {
    ...
    'url': 'https://github.com/baahl-nyu/orion', # Add Orion URL
    'packages': packages,
